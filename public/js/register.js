...
var _require = __webpack_require__(/*! ./utils */ "./resources/js/utils.js"),
    ...
    deleteChild = _require.deleteChild;
...
function getData(route, el, before) {
  if (memoizeData[route]) {
    ...
    var _iterator = _createForOfIteratorHelper(memoizeData[route]),
        _step;
  ...
  window.axios.get("http://localhost:8001/api/" + route).then(function (res) {
    before();
    ...
    try {
      for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
        ...
        el.appendChild(createElement("option", _data.id, _data.name));
      }
    } catch (err) {
      _iterator2.e(err);
    } finally {
